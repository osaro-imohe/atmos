...
export const LotInfo = ({
  ...
  return (
    <>
      ...
      <Box>
        <Text color="secondary" size="sm" text={`${acres}, ${convertAcreToSqft(acres)}`} />
      </Box>
      <Box marginTop="10px">
...
export const HomeInfo = ({
  ...
  <>
    ...
    <Box>
      <Text color="secondary" size="sm" text={`${numBeds} beds - ${numBaths} baths - ${sqft} sqft`} />
    </Box>
    <Box>
      {tags.map((tag) => (
        <Box paddingLeft="2px" paddingRight="2px" paddingTop="2px" paddingBottom="2px" border marginRight="2px" borderRadius="8px">
          <Text color="secondary" size="sm" text={tag} key={Math.random()} />
        </Box>
      ))}
    </Box>
    <Box marginTop="10px">
